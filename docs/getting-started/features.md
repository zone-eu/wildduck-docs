...
![WildDuck Features](/img/sprites/sorting_mail.png)
...
4.  **Centralized authentication** which allows modern features like 2FA (TOTP and WebAuthn/FIDO2), application specific passwords, authentication scopes, revoking authentication tokens,
...
8.  Access messages both using **IMAP and HTTP API**. The latter serves parsed data, so no need to fetch RFC822 messages and parse
...
14. **Extra security features** like automatic GPG encryption of all stored messages or authenticating with WebAuthn/FIDO2
